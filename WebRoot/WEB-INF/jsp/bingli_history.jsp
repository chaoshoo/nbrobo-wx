...
<header class="navbar">
  ...
  <span class="nav-header h1"> 病历列表 </span> 
...
//	alert(title);
	$.ajax({
		url:"<%=path%>/vip/binglilist.json?ttt="+new Date().getTime(),
		type:"get",
		dataType:"json",
		data:"pageNo="+pagenum+"&exam_date="+title,
		success:function(data){
			var liuyanlisthtml = $("#liuyanlist").html();
			var x = 0;
			$.each(data, function(i, item){  
				x = x+1;
				var title = item.EXAM_HOS;
				var answer_content = item.EXAM_SUMUP;				
				if(title ==undefined){
					title = "";
				}
				...
				var create_time = item.EXAM_DATE;
...
function question(id){
	//window.location='<%=path %>/vip/binglidetail.html?id='+id;
}
</script>
</body>
</html>